...
// MatchingResult.js (example file)

import React from "react";
import "./MatchingResult.css"; // connect to the css

const MatchingResult = ({ selectedCompany, score, topCompanies, companyLogos }) => {
  return (
    <div className="matching-result-container">
      ...
      {selectedCompany && (
        <div className="selected-company-section">
          <img
            src={companyLogos[selectedCompany]}
            alt={selectedCompany}
            className="selected-company-logo"
          />
          <h2 className="selected-company-name">{selectedCompany}</h2>
          <p className="matching-score">
            Your Matching <span className="score-highlight">{score}%</span> Score
          </p>
        </div>
      )}

      {/* Top Matching Companies */}
      <h3 className="top-matching-heading">Top Matching Companies</h3>
        ...
        {topCompanies.map((company, index) => (
          <div key={index} className="company-card">
            ...
            <div className="company-info">
              ...
              <p className="company-match">{company.match.toFixed(2)}%</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="tips-section">
        <h3 className="tips-heading">🔧 Tips to Improve Your Match</h3>
